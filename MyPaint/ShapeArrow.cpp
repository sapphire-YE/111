...
void ShapeArrow::paintShape(QPainter *painter)
{
  ...
  // 计算箭头点
  QPoint arrowP1 = m_line.p2() - QPoint(arrowSize * cos(angle + M_PI / 6),
                                        arrowSize * sin(angle + M_PI / 6));
  QPoint arrowP2 = m_line.p2() - QPoint(arrowSize * cos(angle - M_PI / 6),
                                        arrowSize * sin(angle - M_PI / 6));
...
bool ShapeArrow::contains(const QPoint &pt) const
{
  ...
  if (m_rotation == 0.0)
  {
    ...
    double projection =
        toPoint.x() * lineNormalized.x() + toPoint.y() * lineNormalized.y();
...
void ShapeArrow::moveBy(const QPoint &delta) { m_line.translate(delta); }
...
std::vector<ShapeBase::Handle> ShapeArrow::getHandles() const
{
  ...
  // 起点锚点
  handles.push_back({QRect(m_line.p1().x() - size / 2,
                           m_line.p1().y() - size / 2, size, size),
                     Handle::Scale, 0});

  // 终点锚点
  handles.push_back({QRect(m_line.p2().x() - size / 2,
                           m_line.p2().y() - size / 2, size, size),
                     Handle::Scale, 1});
  ...
  handles.push_back({QRect(rect.center().x() - rotateSize / 2,
                           rect.top() - rotateOffset - rotateSize / 2,
